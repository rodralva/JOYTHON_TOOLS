...
builtins.__dict__["SBNDSTYLE_ENABLE_AUTOMATICALLY"] = False
import dunestyle
```
...
def enable():
    ...
    path = os.path.join(os.environ['MPLCONFIGDIR'].split(os.pathsep)[0], "SBND.mplstyle")
    assert os.path.exists(path), "Can't locate DUNE matplotlib style sheet file!  I tried path: " + path
    plt.style.use(path)
    print("SBND plot style enabled")


# import builtins
# _IMPORT_FLAG_NAME = "SBNDSTYLE_ENABLE_AUTOMATICALLY"
...
def SBNDWatermarkString():
    """
    Produces the "SBND" part of the string used in watermarks so it can be styled independently if necessary
    :return: The appropriately styled "SBND" string
    """

    return r"$\mathdefault{\bf{SBND}}$"

def Preliminary(x=0.05, y=0.90, align='left', transform=None, ax=None, **kwargs):
    """
    Apply a "SBND Preliminary" label.
    ...
    :return:           None
    """
    TextLabel(SBNDWatermarkString() + " Preliminary", x, y, ax=ax, transform=transform, align=align, color="black", **kwargs)

def WIP(x=0.05, y=0.90, align='left', transform=None, ax=None, **kwargs):
    """
    Apply a "SBND Work In Progress" label.

    See help on TextLabel() for the optional parameters.
    """
    TextLabel(SBNDWatermarkString() + " Work In Progress", x, y, ax=ax, transform=transform, align=align, color="black", **kwargs)

def Simulation(x=0.05, y=0.90, align='left', ax=None, transform=None, **kwargs):
    """
    Apply a "SBND Simulation" label.

    See help on TextLabel() for the optional parameters.
    """
    TextLabel(SBNDWatermarkString() + " Simulation", x, y, ax=ax, transform=transform, align=align, color="black", **kwargs)

def SimulationSide(x=1.05, y=0.5, align='right', ax=None, transform=None, **kwargs):
    """
    Apply a "SBND Simulation" label on the right outside of the frame.
    ...
    """
    TextLabel(SBNDWatermarkString() + " Simulation", x, y, ax=ax, transform=transform, align=align, rotation=270, color="black", **kwargs)

def Official(x=0.05, y=0.90, align='left', ax=None, transform=None, **kwargs):
    """
    Apply a "SBND" label (for officially approved results only).

    See help on TextLable() for the optional parameters.
    """
    TextLabel(SBNDWatermarkString(), x, y, ax=ax, transform=transform, align=align, color="black", **kwargs)
